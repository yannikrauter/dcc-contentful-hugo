...
(the folder called `hugo-contentful-static`), run:
```
npm install
```
...
(and content). The content model can be imported via the `contenful-cli` from the
...
`ACCESS_TOKEN`, `PREVIEW_ACCESS_TOKEN` and `SPACE_ID`.
These can be sourced from Contentful.

Again, in the root directory of the repository, run:
```
npx gulp
```
